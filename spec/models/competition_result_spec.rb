...
describe CompetitionResult do
  ...
  let(:competition_result) { FactoryBot.build(:competition_result) }
  ...
  context "with a custom name" do
    let(:competition_result) { FactoryBot.build(:competition_result, system_managed: true, name: "Fun Results") }
  ...
  context "as system managed without a custom name" do
    let(:competition_result) { FactoryBot.build(:competition_result, system_managed: true, name: nil) }
