  (function( $ ) {
    $.widget( "ui.combobox", {
      ...
      _removeIfInvalid: function( event, ui ) {
        ...
        var first_match_option = null;
        var matcher = new RegExp( $.ui.autocomplete.escapeRegex(valueLowerCase), "i" );
        this.element.children( "option" ).each(function() {
          var text = $(this).text().toLowerCase();
          if ( this.value && ( matcher.test(text) ) ) {
            count_matching += 1;
            first_match_option = this;
          }
        });
        if (count_matching == 1) {
          this.input.val($(first_match_option).text());
          first_match_option.selected = true;
          valid = true;
          return false;
        }
