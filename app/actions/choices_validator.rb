class ChoicesValidator
  ...
  def valid_event_choice_registrant_choice(event_selected, event_choice, reg_choice) # rubocop:disable Metrics/CyclomaticComplexity
    ...
    if event_selected && !valid_with_required_selection(event_choice, reg_choice_chosen)
      @registrant.errors.add(:base, "#{event_choice} must be specified if #{required_if_event_choice} is chosen")
      return false
    end

    if event_selected && !valid_with_optional_selection(event_choice, reg_choice_chosen)
      @registrant.errors.add(:base, "#{event_choice} must be specified unless #{optional_if_event_choice} is chosen")
      return false
    end
